...
function diffCharsIfWithinEditLimit(
  ...
  return {
    exceededLimit: false,
    diffs: diffs.map((d) => {
      let type: "insert" | "delete" | "normal" = "normal";
      if (d.added) {
        type = "insert";
      } else if (d.removed) {
        type = "delete";
...
const buildInlineDiffSegments = (
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (currentSegment.type === "delete" && nextSegment?.type === "insert") {
      ...
      mergeIntoResult(currentSegment);
    }
  }
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    if (
      ...
      out.push({
        ...
        type: "normal",
        isNormal: true,
        oldLineNumber: (current as DeleteChange).lineNumber,
...
function buildUnpairedDeletePrefix(changes: _Change[], pairOfDel: Int32Array) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const isInitiallyUnpairedDelete =
      c && c.type === "delete" && pairOfDel[i] === UNPAIRED;
    const prevValue = prefix[i];
    if (prevValue !== undefined) {
      prefix[i + 1] = prevValue + (isInitiallyUnpairedDelete ? 1 : 0);
    }
  }
...
function hasUnpairedDeleteBetween(
  ...
  const upperValue = unpairedDelPrefix[upper];
  const lowerValue = unpairedDelPrefix[lower];
  return (
...
function processDeleteChange(
  ...
  const add = changes[pairedAddIdx];
  if (!add) {
    return;
  }
  emitModified(out, c as DeleteChange, add as InsertChange, options);
  processed[i] = 1;
  processed[pairedAddIdx] = 1;
}

function processInsertChange(
  i: number,
  changes: _Change[],
  pairOfAdd: Int32Array,
  processed: Uint8Array,
  ...
  const del = changes[pairedDelIdx];
  if (!del) {
    return;
  }
  emitModified(out, del as DeleteChange, c as InsertChange, options);
  processed[i] = 1;
...
function emitLines(
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    if (c.type === "normal") {
      ...
      processInsertChange(i, changes, pairOfAdd, processed, out, options);
    }
  }
